...
import net.yebaihe.puzzle.MultiTouchController.MultiTouchObjectCanvas;
import net.yebaihe.puzzle.MultiTouchController.PointInfo;
import net.yebaihe.puzzle.MultiTouchController.PositionAndScale;
...
public class PhotoSorterView extends View implements MultiTouchObjectCanvas<PhotoSorterView.Img> {
	...
	public PhotoSorterDelegate delegate;
	...
	public class Img {
		...
		public void load(Resources res) {
			...
			this.width =  bitmap.getWidth(); //drawable.getIntrinsicWidth();
			this.height = bitmap.getHeight(); //drawable.getIntrinsicHeight();
			/*float cx, cy, sc;
			if (firstLoad) {
	...
	//private static final int[] IMAGES = { R.drawable.i01, R.drawable.i02, R.drawable.i03};
	...
	private ArrayList<Img> fixedImg=new ArrayList<Img>();
	...
	public boolean setPositionAndScale(Img img, PositionAndScale newImgPosAndScale, PointInfo touchPoint) {
		if (fixedImg.indexOf(img)>=0) return false;
		...
		if (ok){
			if (delegate!=null) delegate.imagePositionChange(img,x,y,touchPoint);
			invalidate();
		}
		return ok;
	}
    ...
    {  
		if (delegate!=null) delegate.sizeChanged();
    }
	
	public void addImage(Bitmap bitmap, Resources resources,float x,float y) {
		Img i=new Img(bitmap, resources);
		mImages.add(i);
		i.load(resources);
		i.setPos(x, y, 1);
		invalidate();
		if (delegate!=null) delegate.imagePositionChange(i,x,y,null);
	}

	public void setImgPosition(Img i, int x, int y, boolean fixed) {
		i.setPos(x, y,1);
		invalidate();
		if (fixed){
			if (fixedImg.indexOf(i)<0){
				fixedImg.add(i);
				mImages.remove(i);
				mImages.add(0, i);
			}
